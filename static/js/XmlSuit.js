<%if request.querystring="v=1.0.0" then    server.transfer("XmlSuit.js_v=1.0.0") %>﻿/*-------------------------------------------------------------------------
 * 2017-2-27 by ligen
 * 独立道具xml解析
 -------------------------------------------------------------------------*/
                ...
                for (i; i < aLength; i++) {
                    ...
                    if (bLength > 0) {
                        ...
                        if (nowEleNodeName == "icon") {
                            obj['icon'] += "<li class='js_suit' data-suitidx=" + suitindex + "><img class='' src='/img.gyyxcdn.cn/qibao/Images/itemImages/" + gy.getxmlnodeText(nowEle) + ".jpg' style='width:50px;height:50px;'></li>";
                            suitindex++;
                        }
                    }
                }
