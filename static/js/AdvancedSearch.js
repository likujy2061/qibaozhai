<%if request.querystring="v=3.3.3" then    server.transfer("AdvancedSearch.js_v=3.3.3") %>﻿/*add by ligen 2016.8.22*/
$(function () {
        ...
        if (!$(this).hasClass("searchItemDown")) {
            ...
            for (var i = 0; i < $(".js_menu_sub1 a").length; i++) {
                if ($(".js_menu_sub1 a").eq(i).hasClass("curr")) {
                    $(".secondarypoint").show();
                    $(".js_searchItemPanel").animate({ height: $(".js_menu_sub1 a").eq(i).attr("data-height") });
                }
            }
        }
    });
    ...
    $(".js_searchbtn").die("click").live("click", function () {
        searchAllServer($(".js_searchbtn").attr("data-hsl"));
    });
    /*重置搜索条件按钮*/
    ...
    function checkRoleForm() {
        ...
        if (!ValidateInterval("roleMinSkill", "roleMaxSkill", "道行", regCount, 10, 1000000000, "true")) { //验证道行
            return false;
        }
        if (!ValidateInterval("roleMinTiZhi", "roleMaxTiZhi", "体质加点", regCount, 0, 10000, "false")) { //验证体质加点
            return false;
        }
        if (!ValidateInterval("roleMinLingLi", "roleMaxLingLi", "灵力加点", regCount, 0, 10000, "false")) { //验证灵力加点
            return false;
        }
        if (!ValidateInterval("roleMinLiLiang", "roleMaxLiLiang", "力量加点", regCount, 0, 10000, "false")) { //验证力量加点
            return false;
        }
        if (!ValidateInterval("roleMinMinJie", "roleMaxMinJie", "敏捷加点", regCount, 0, 10000, "false")) { //验证敏捷加点
            return false;
        }
        if (!ValidateInterval("roleMinXianDao", "roleMaxXianDao", "仙道点", regCount, 0, 10000, "false")) { //验证仙道点
            return false;
        }
        if (!ValidateInterval("roleMinMoDao", "roleMaxMoDao", "魔道点", regCount, 0, 10000, "false")) { //验证魔道点
            return false;
        }
        if (!ValidateInterval("roleMinLevel", "roleMaxLevel", "等级", regCount, 1, 200, "true")) { //验证等级加点
            return false;
        }
        return true;
    ...
    //搜索点击事件执行函数
    function searchAllServer(isshowbtn) {
        if ($("#ServerList").val() == 0) {
            alert("您还未选择想要搜索区服，请先选择后再进行搜索");
        } else {
            if (searchtype == 0) {
                if (checkRoleForm()) {
                    selstatefn(".js_selstate", ".js_selrolAll");
